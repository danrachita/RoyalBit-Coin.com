...
#if defined(NDEBUG)
#error "ROBI cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "ROBI Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
	...
	if(nHeight <= 500 && nHeight > 0) {
		...
		nSubsidy = 150 * COIN;
	} else if (nHeight > 25000 && nHeight <= 75000) {
		nSubsidy = 70 * COIN;
	} else if (nHeight > 75000 && nHeight <= 110000) {
		nSubsidy = 50 * COIN;
	} else if (nHeight > 110000 && nHeight <= 160000) {
		nSubsidy = 25 * COIN;
	} else if (nHeight > 160000 && nHeight <= 200000) { // 200000 => LAST POW BLOCK
		nSubsidy = 10 * COIN;
	} else if (nHeight > 200000 && nHeight <= 250000) { // 200001 => FIRST POS BLOCK
		nSubsidy = 150 * COIN;
	} else if (nHeight > 250000 && nHeight <= 300000) {
		nSubsidy = 50 * COIN;
	} else if (nHeight > 300000 && nHeight <= 900000) {
		nSubsidy = 10 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	if(nHeight <= 500 && nHeight > 0) {
		...
		if (mNodeCoins == 0) {
			...
			LogPrintf("[LIBRA] Adjusting Libra at height %d with %d masternodes (%d % locked ROBI) and %d ROBI supply at %ld\n", nHeight, nMasternodeCount, lockedCoinValue*100, nMoneySupply, GetTime());

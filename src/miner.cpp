...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ROBIMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("ROBIMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("ROBIMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("robi-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("ROBIMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running ROBIMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadROBIMiner() exception");
    } catch (...) {
        LogPrintf("ThreadROBIMiner() exception");
    }

    LogPrintf("ThreadROBIMiner exiting\n");
}
